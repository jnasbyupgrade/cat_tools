...
SELECT __cat_tools.create_function(
  ...
  , 'cat_tools.routine_type LANGUAGE sql STRICT IMMUTABLE PARALLEL SAFE'
  , $body$
SELECT cat_tools.routine__type(prokind::cat_tools.routine_prokind)
$body$
  , 'cat_tools__usage'
  , 'Mapping from pg_proc.prokind to cat_tools.routine_type'
);
...
SELECT __cat_tools.create_function(
  ...
  , 'cat_tools.routine_argument_mode LANGUAGE sql STRICT IMMUTABLE PARALLEL SAFE'
  , $body$
SELECT cat_tools.routine__argument_mode(proargmode::cat_tools.routine_proargmode)
$body$
  , 'cat_tools__usage'
  , 'Mapping from pg_proc.proargmodes element to cat_tools.routine_argument_mode'
);
...
SELECT __cat_tools.create_function(
  ...
  , 'cat_tools.routine_volatility LANGUAGE sql STRICT IMMUTABLE PARALLEL SAFE'
  , $body$
SELECT cat_tools.routine__volatility(provolatile::cat_tools.routine_provolatile)
$body$
  , 'cat_tools__usage'
  , 'Mapping from pg_proc.provolatile to cat_tools.routine_volatility'
);
...
SELECT __cat_tools.create_function(
  ...
  , 'cat_tools.routine_parallel_safety LANGUAGE sql STRICT IMMUTABLE PARALLEL SAFE'
  , $body$
SELECT cat_tools.routine__parallel_safety(proparallel::cat_tools.routine_proparallel)
$body$
  , 'cat_tools__usage'
  , 'Mapping from pg_proc.proparallel to cat_tools.routine_parallel_safety'
);
...
BEGIN
  ...
  IF current_setting('server_version')::real >= 11.0 THEN
    v_execute_clause := ' EXECUTE FUNCTION ' || r_trigger.tgfoid::pg_catalog.regproc || E'\\(';
  ELSE
